...
def  add_noise(lr,mode,noise_level):
    ...
    #certain noise, i.e., 0.25,35,50,75
       noise = (lr.size()).normal_(mean=0,std=noise_level/255.)
    if mode == 0:
    #blind noise, i.e., 0-55
       noisel_b = [0,55]
       noise_size = lr.size()
       noise = np.zeros(noise_size)
       stdN = np.random.uniform(noisel_b[0],noisel_b(1),size=noise_size()[0])
       for n in range(noise.size()[0]):
           sizeN = noise[0,:,:,:].size()
           noise[n,:,:,:] = noise_size.normal_(mean=0,std=stdN[n]/255.)
    lr_noise = lr + noise
    print lr, lr_noise
''' 
